...
# 把 uploadVoice 方法改回来，真正调用 voice-enrollment API
    ...
    public ResponseEntity<?> uploadVoice(@PathVariable Long id,
        ...
        try {
            ...
            try {
                ...
                String voiceId = voiceClient.createVoice(audioUrl, "role" + id);
...
print('Restored real uploadVoice method')

# 把 tts 方法也改回来，去掉演示模式的直接返回
old_tts_demo = '''            // 演示模式：如果角色上传了声音样本，直接返回预设的老奶奶音频
            // 比赛演示用，不需要真正的 TTS 克隆
            ...
            // 优先用自定义声音'''
new_tts_demo = '''            // 优先用自定义声音'''
content = content.replace(old_tts_demo, new_tts_demo)
print('Removed demo mode from tts method')
...
with open(r'C:\Users\han\ClawAssistant\src\main\java\com\youkeda\exercise\claw\role\AiRoleController.java', 'w', encoding='utf-8') as f:
    f.write(content)
print('AiRoleController.java - restored real voice cloning')
